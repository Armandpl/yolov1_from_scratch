...
def batch_iou(pred_boxes, target_boxes):
    ...
    for b_idx in range(bs):
        ious[b_idx] = ops.box_iou(target_boxes[b_idx],
                                  pred_boxes[b_idx]).diag()
...
class YoloLoss(torch.nn.Module):
    ...
    def forward(self, predictions, target):
        ...
        pred_boxes = obj_mask * \
            (ious_mask_box_1 * pred_boxes_1 + ious_mask_box_2 * pred_boxes_2)
        ...
        center_loss = self.mse(pred_boxes[..., :-3],
                               target_boxes[..., :-3]) * self.lambda_coords
        dimensions_loss = self.mse(dimensions, target_dimensions)\
            * self.lambda_coords

        coords_loss = center_loss + dimensions_loss

        # obj/noobj loss
        obj_pred = (predictions[..., -5]*ious_mask_box_1_ +
                    predictions[..., -10]*ious_mask_box_2_)
        obj_loss = self.mse(obj_mask_ * obj_pred, target[..., -5])

        noobj_mask = torch.ones_like(obj_mask_) - obj_mask_
        noobj_loss = self.mse(noobj_mask * obj_pred, target[..., -5]) \
            * self.lambda_noobj

        # classes loss
        obj_mask = obj_mask_.unsqueeze(3).expand(-1, -1, -1, self.n_classes)
        classes_loss = self.mse(predictions[..., :-10]
                                * obj_mask, target[..., :-5])
